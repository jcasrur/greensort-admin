...
const NavItem = ({ icon, label, path, currentPath, navigate, t, badge, isLightMode }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`text-sm flex-1 truncate ${active ? 'font-semibold' : 'font-normal'}`}>
        {label}
      </span>

      {badge && (
        <span
          className={`text-[9px] font-bold px-2 py-0.5 rounded-full tracking-wider flex-shrink-0 ${
            badge === 'SA'
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/users', label: 'User Management', icon: Icons.users, module: 'user_management' },
...
export default function Sidebar() {
  ...
  const { adminUser, role, roleLabel } = useAdminAuth();
  ...
  return (
    <div
      className={`w-56 h-full ${
        isLightMode ? 'bg-white border-[#E8F0E9]' : 'bg-[#0A140A] border-white/[0.06]'
      } border-r flex flex-col shrink-0 transition-colors duration-300`}
    >
      <div className="px-5 py-5 flex items-center gap-2.5">
        <div
          className={`w-7 h-7 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0 ${
            isLightMode ? 'bg-[#D8EDDF]' : 'bg-[#5E9E7A]/20'
          }`}
        >
          <img
        ...
        <div>
          <h1
            className={`text-sm font-semibold tracking-tight ${
              isLightMode ? 'text-[#1A2A1A]' : 'text-[#E8F0E5]'
            }`}
          >
            GreenSort
          </h1>
          <p
            className={`text-[9px] tracking-widest uppercase ${
              isLightMode ? 'text-[#4A5D4E]' : 'text-[#A8BDA2]'
            }`}
          >
            Admin Panel
          </p>
      ...
      <nav className="flex-1 overflow-y-auto px-3 space-y-0.5 pb-3" style={{ scrollbarWidth: 'none' }}>
        {/* TESTING ONLY:
            Lahat ng sections ay visible muna kahit anong admin role.
            Kapag final na, ibalik yung can(item.module) filtering. */}
        {NAV_GROUPS.map((group) => {
          ...
          return (
            <div key={group.label}>
              <p
                className={`text-[9px] font-semibold uppercase tracking-widest px-3 py-2 ${
                  isLightMode ? 'text-[#5E7A67]' : 'text-[#7A8C77]'
                }`}
              >
                {group.label}
              </p>
        ...
        {/* TESTING ONLY:
            Admin Access visible muna kahit hindi super_admin. */}
      ...
      <div className={`p-3 border-t ${borderCls} space-y-2`}>
        <button
          ...
          className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs transition-all ${
            isLightMode
              ? 'text-[#4A5D4E] hover:bg-[#F3F6F1]'
              : 'text-[#A8BDA2] hover:bg-white/[0.04]'
          }`}
        >
        ...
        <div
          className={`flex items-center gap-2.5 p-2.5 rounded-xl ${
            isLightMode ? 'bg-[#F3F6F1]' : 'bg-white/[0.03]'
          }`}
        >
          <div
            className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0 ${
              isLightMode ? 'bg-[#D8EDDF] text-[#1A2A1A]' : 'bg-[#007C00]/20 text-[#00E676]'
            }`}
          >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-[9px] font-medium ${roleDotCls}`}>
              {roleLabel}
            </p>
          </div>
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-all ${
              isLightMode
                ? 'text-[#5E7A67] hover:text-red-500 hover:bg-red-50'
                : 'text-[#A8BDA2] hover:text-red-400 hover:bg-red-400/10'
            }`}
            title="Sign out"
          >
