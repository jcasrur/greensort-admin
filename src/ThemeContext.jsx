...
export const ThemeProvider = ({ children }) => {
  ...
  const t = {
    ...
    sidebarBg:  isLightMode ? 'bg-white'        : 'bg-[#0A140A]',

    // Text
    textMain:   isLightMode ? 'text-[#1A2A1A]' : 'text-white',
    textSub:    isLightMode ? 'text-[#2D4A38]' : 'text-[#C4D9CC]',
    ...
    accentHex:  isLightMode ? '#007C00'          : '#2CD87D',

    // Inner / utility
    innerBg:    isLightMode ? 'bg-[#F2F5F2]' : 'bg-[#070F07]',
    divider:    isLightMode ? 'bg-[#E8F0E9]' : 'bg-white/[0.05]',

    // Chip — small pill background (e.g. "This Week" badge)
    chipBg:     isLightMode ? 'bg-[#D8EDDF]' : 'bg-[#1C2C1C]',
